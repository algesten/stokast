#![allow(dead_code)]
//! Driver for Max6959 segment LED controller.
//! Datasheet here: https://datasheets.maximintegrated.com/en/ds/MAX6958-MAX6959.pdf
...
/// Max6959 address variant.
///
/// It's possible to have two max6959 on the same bus, in which case one
...
where
    ...
    I: WriteRead<Error = E>,
{
    pub fn new(i2c: I, variant: Variant) -> Self {
        Max6959 {
            i2c,
            addr: variant as u8,
        }
    ...
    pub fn set_shutdown(&mut self, shutdown: bool) -> Result<(), E> {
        info!("set_shutdown: {}", shutdown);
        let config_r = self.read_register(Register::Configuration)?;
    ...
    pub fn set_display_test(&mut self, test: bool) -> Result<(), E> {
        info!("set_diplay_test: {}", test);
        self.write_register(Register::DisplayTest, if test { 1 } else { 0 })
    }
    ...
    pub fn set_scan_limit(&mut self, limit: ScanLimit) -> Result<(), E> {
        info!("set_scan_limit: {:?}", limit);
        self.write_register(Register::ScanLimit, limit as u8)
    }

    pub fn set_decode_mode(&mut self, decode: &[Digit]) -> Result<(), E> {
        ...
        self.write_register(Register::DecodeMode, mode)
    }
    ...
    fn write_register(&mut self, reg: Register, data: u8) -> Result<(), E> {
        self.i2c.write(self.addr, &[reg.addr(), data])
    }
